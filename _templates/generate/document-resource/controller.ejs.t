...
import { FindAll<%= h.inflection.transform(name) %>Dto } from './dto/find-all-<%= h.inflection.transform(name, ['pluralize', 'underscore', 'dasherize']) %>.dto';
...
export class <%= h.inflection.transform(name) %>Controller {
  constructor(private readonly <%= h.inflection.camelize(h.inflection.pluralize(name), true) %>Service: <%= h.inflection.transform(name) %>Service) {}
  ...
  async findAll(
    @Query() query: FindAll<%= h.inflection.transform(name) %>Dto,
